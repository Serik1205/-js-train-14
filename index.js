...
function nextNewYear() {
	...
	let potYear = date.getFullYear();
	const nextYearDate = new Date(potYear + 1, 0, 1);
	let diff = nextYearDate.getTime() - dayOfMonth.getTime();
...
function addDays(date, days) {
	...
	let addedDays = date.setTime(days);
	let resultDate = date.toISOString(date.setDate(days));
	return {
		inputDate,
		addedDays,
		resultDate
...
function getDaysInMonth(date) {
	...
	let inputDate = date.toISOString();
	let potMonth = date.getMonth();
	let potYear = date.getFullYear();
	let nextMonth = new Date(potYear, potMonth + 1, 1);
...
function getFormattedTime(date) {
	...
	let formattedTime = date.getTime(`${newHours.toString().padStart(2, "0")}:${newMinutes.toString().padStart(2, "0")}:${newSeconds.toString().padStart(2, "0")}`)
	return {
		inputDate,
		formattedTime
	}
...
function compareDates(date1, date2) {
	...
	if (!(typeof date2.getTime === "function")) {
		...
		return { date1: compareDate1, date2: compareDate2, comparison };
	};
}
...
function getDaysDifference(startDate, endDate) {
	...
	let timeDifferense = endTime - startTime;
	let daysDifference = timeDifferense / (1000 * 60 * 60 * 24)
	let formattedStartDay = startDate.toISOString();
	let formattedEndDay = endDate.toISOString();
